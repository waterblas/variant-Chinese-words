...
SearchBox.prototype.extract = function() {
	...
	sqlite.findAll(idCondition, function(err, row){
		...
		if(checkbox["xingjinCheckbox"]){
			findXingjin(sqlite, hanziIds, that.variant["xingjin"]);
		}
		if(checkbox["tongyinCheckbox"]){
			findTongyin(sqlite, hanziIds, that.variant["tongyin"]);
		}
		if(checkbox["chaifenCheckbox"]){
			findChaifen(sqlite, hanziIds, that.variant["chaifen"]);
		}
...
function findXingjin(sqlite, hanziIds, variant){
		...
		sqlite.findEach(xingjinCondition, function(err_l,row_l){
			...
			for(var i in variant){
				if(variant[i].category == row_l.category){
					variant[i].xingjinId.push(row_l.id);
				}
			}
		})
...
function findTongyin(sqlite, hanziIds, variant){
	var pinyinCondition = {
			"table": "hanzi_pinyin",
			"fields": ["id","pinyin"],
			"conditions": {
...
function findChaifen(sqlite, hanziIds, variant){
