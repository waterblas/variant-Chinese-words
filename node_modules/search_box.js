...
SearchBox.prototype.formatDate = function(){
	var hanziName = this.hanziNameById;
	var variant = this.variant;
	var formatDate = new Object();
	var tongyinzi = [];
	var hanziIds = [];
	for(var i in hanziName){
		formatDate[i] = {"chaifen":null, "xingjin":null, "tongyin":null};
		if(variant.tongyin){
	...
	sqlite.findAll(nameCondition, function(err, row){
		...
		if(variant.chaifen){
			for(var i in hanziName){
				formatDate[i].chaifen = (variant.chaifen)[hanziName[i]];
			}
		}
		if(variant.tongyin){
			for(var i in hanziName){
