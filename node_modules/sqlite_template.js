...
var langTemplate = function(databasePath) {
	...
	var preSql = function(con){
		...
		if(Array.isArray(con.fields)){
			fieldName = con.fields.join(',');
		}else{
			fieldName = con.fields;
		}
		return "SELECT " + fieldName + " FROM " + tableName + " WHERE 1 AND " + connectLang(con.conditions);
	};
	...
	return {
		...
		findEach: function(con, fun,fun2){
			var sqlang = preSql(con);
			console.log(sqlang);
			db.each(sqlang, fun, fun2);
		}
	}
}
